...
// Function to map old keys to new structure
function mapOldToNew(key, oldTranslations) {
  const mapping = {
    ...
    // Metadata
    "home-title": oldTranslations?.Metadata?.["home-title"],
    "home-description": oldTranslations?.Metadata?.["home-desc"],
    "about-title": oldTranslations?.Metadata?.["about-title"],
    "about-description": oldTranslations?.Metadata?.["about-desc"],
    "pricing-title": oldTranslations?.Metadata?.["pricing-title"],
    "pricing-description": oldTranslations?.Metadata?.["pricing-desc"],
    "features-title": oldTranslations?.Metadata?.["features-title"],
    "features-description": oldTranslations?.Metadata?.["features-desc"],
    "login-title": oldTranslations?.Metadata?.["log-title"],
    "login-description": oldTranslations?.Metadata?.["log-desc"],
    "not-found-title": oldTranslations?.Metadata?.["notfound-title"],
    "not-found-description": oldTranslations?.Metadata?.["notfound-desc"],
...
// Function to recursively fill the structure
function fillStructure(structure, oldTranslations) {
  const result = {};
  
  for (const [key, value] of Object.entries(structure)) {
    if (typeof value === 'object' && value !== null) {
      result[key] = fillStructure(value, oldTranslations);
    } else {
...
files.forEach(file => {
  ...
  const newTranslations = fillStructure(newStructure, oldTranslations);
  
  // Write the new organized file
  const newFilePath = path.join(messagesDir, `new-${file}`);
...
console.log('📁 New files created with prefix "new-"');
console.log('\nNext steps:');
console.log('1. Review the new structure');
console.log('2. Update your code to use the new keys');
